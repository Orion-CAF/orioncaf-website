...
export default function Hero({ dict }: { dict: any }) {
  return (
    <section className="relative text-center px-6 md:px-16 pt-20 md:pt-28 pb-16 md:pb-24 max-w-[860px] mx-auto overflow-hidden">
      ...
      <div className="inline-flex items-center gap-2 text-[13px] font-semibold text-accent bg-green-50 px-4 py-1.5 rounded-full mb-8">
        <div className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse"></div>
        {dict.badge}
      </div>
      <h1 className="text-[38px] md:text-[56px] font-bold leading-[1.08] tracking-[-2px] text-[#1a1a1a] mb-6">
        {dict.title1}<br />
        {dict.title2}<em className="not-italic bg-gradient-to-r from-accent to-[#6B9B2A] bg-clip-text text-transparent">{dict.titleHighlight}</em>
      </h1>
